...
- **`/behavior-verification`**: For **feature-level** testing of individual
  implementations (generates verification checklists, guides user through testing).
  Use `/behavior-verification` after implementing a feature to verify it works correctly.
  This skill (`/gate-check`) is for **project-phase** validation.
- **`/implementation-workflow`**: After completing an implementation workflow,
  suggest running `/gate-check` if the feature completes a phase milestone.
