---
name: design-system
description: "Unified design skill: GDD authoring for game systems (section-by-section with dependency tracking) AND general-purpose design discussion for implementation tasks. Integrates with coding-standards for language-specific conventions."
argument-hint: "[system-name or feature-description] (e.g., 'combat-system', 'login-feature', or omit for interactive mode)"
user-invocable: true
...
This skill operates in two modes based on the argument provided:

### Mode Detection

1. **If argument matches a game system name** (kebab-case, matches entries in
   `design/gdd/systems-index.md`, or is clearly a game mechanic like "combat",
   "inventory", "dialogue", "progression"):
   → Enter **GDD Mode** (Section 2 onwards)

2. **If argument is a general feature/task description** (e.g., "login-feature",
   "search API", "ログイン機能") OR **no argument is provided**:
   → Enter **General Design Mode** (Section 1b)

### GDD Mode Argument Validation

Normalize the system name to kebab-case for the filename (e.g., "combat system"
becomes `combat-system`).

If no argument is provided and GDD mode is detected via context, ask the user:
> "Which system would you like to design? Run `/map-systems` to see available systems."

---

## 1b. General Design Mode (Non-GDD)

For general implementation tasks that don't require a full 8-section GDD.
Follows a streamlined 3-stage workflow.

### Prefix Extraction

Extract a file prefix from the feature description:
1. Identify the main noun or concept (e.g., "ログイン機能の実装" → `login`)
2. Convert to lowercase, replace spaces with underscores
3. Use ASCII-safe characters only, max 20 characters
...
4. What are the constraints? (language, framework, performance)
5. What are the success criteria?

**Load Coding Standards** (see Section 2e for details).

Generate 5-10 clarifying questions about edge cases, error handling, and
integration points.

**Exit condition:** Clear understanding of requirements, constraints, and criteria.
...
- **Fit**: How well it matches constraints

For key architectural decisions, present options with trade-offs and capture
decisions via `AskUserQuestion`.

Iterate on the chosen approach: drill into components, clarify interfaces,
address edge cases, consider testing strategy.
...
- Reference: See `/coding-standards` skill
...
**File Output:** Save to `{prefix}_design_output.md` (ask user for location).

After generating, return to the main flow. If called from `/implementation-workflow`,
output to the path specified by the workflow.

---

*The following sections (2-7) apply to GDD Mode only.*
...
### 2e: Load Coding Standards

Integrate with the `/coding-standards` skill for language-specific conventions:

1. **Check available standards**: Invoke `/coding-standards` to list available
   language packs (e.g., Unity, C# Project).

2. **Auto-detect or ask**: If the engine is configured in CLAUDE.md (e.g., Unity),
   auto-select the matching standard. Otherwise, present available options:

   Use `AskUserQuestion`:
   - "Which coding standards should apply to this system's implementation?"
     - Options: [list from coding-standards skill], "Skip — use project defaults"

3. **Load and hold**: Read the selected standards from `coding-standards/references/`.
   Hold key conventions (naming, formatting, constraints like "LINQ prohibited")
   in context for the Acceptance Criteria and Implementation Notes sections.

4. **Standards not found**: If the user requests a standard that doesn't exist:
   > "⚠ {Standard} standards are not available. Options:
   > ① Select from existing standards (`/coding-standards`)
   > ② Create new standards (`/coding-standards-creator`)
   > ③ Proceed with project defaults from `.claude/docs/coding-standards.md`"

The loaded standards will be referenced in the GDD's Acceptance Criteria section
and passed to implementation agents when the system moves to coding.
...
[To be configured — loaded via `/coding-standards` skill]
