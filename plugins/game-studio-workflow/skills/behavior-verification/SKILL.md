...
- **`/gate-check`**: For **project-phase** validation (Concept → Production → Release).
  Use `/gate-check` to verify readiness to advance between development phases.
  This skill (`/behavior-verification`) is for **feature-level** testing of individual
  implementations.
- **`/implementation-workflow`**: This skill is invoked at Stage 4 of the implementation
  workflow. After verification passes, the workflow proceeds to `/code-review`.
