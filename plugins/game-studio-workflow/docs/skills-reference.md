...
| `/design-system` | Unified design skill: GDD authoring (section-by-section) AND general-purpose design discussion with coding standards integration |
| `/setup-engine` | Configure engine + version, detect knowledge gaps, populate version-aware reference docs |
| `/implementation-workflow` | Full implementation workflow: design → agent implementation → verification → code review, with progress tracking and checkpoints |
| `/behavior-verification` | Generate verification checklists for implemented features and guide user through testing |
| `/coding-standards` | Load and display language-specific coding standards from dynamic language packs |
| `/coding-standards-creator` | Create new coding standards language packs with 15-section enterprise templates |
